...
def get_config():
    ...
    d_pos = pulp.LpVariable.dicts("d_pos", (i for i in df.index), lowBound=0)
    d_neg = pulp.LpVariable.dicts("d_neg", (i for i in df.index), lowBound=0)

    target_satisfaction = 3  # Define a reasonable target for average satisfaction score

    # Objective Function
    # Minimize the sum of deviations to ensure more consistent satisfaction scores
    model += pulp.lpSum(s[i] for i in df.index) - (pulp.lpSum(d_pos[i] + d_neg[i] for i in df.index))

    # Define individual satisfaction and deviations
    for i in df.index:
        model += s[i] == pulp.lpSum([x[i, df.loc[i, 'primarydorm']] * 2 +
                                     ...
                                     for j in DORMS])
        model += d_pos[i] >= s[i] - target_satisfaction
        model += d_neg[i] >= target_satisfaction - s[i]
