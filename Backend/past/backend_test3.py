...
#                                         ((s['Name'], d['Dorm_Name']) for _, s in df.iterrows() for _, d in df_dormdata.iterrows()if d["Gender"]==s["Gender"]) ,
#                                         cat='Binary')
...
#     # Constraints
#     for _, s in df.iterrows():
#     #     print(pulp.lpSum(assignments[s['Name'], d['Dorm_Name']] for _, d in df_dormdata.iterrows() if d['Dorm_Name'] in s['Dorm_Preferences'])==1)
